...
async function airtableRequest(tableName: string, method: string, data?: any) {
...
async function handleCreateLead(args: any) {
  const record = {
    records: [{
      fields: {
...
async function handleGetLeads(args: any) {
  ...
  if (args.companyType) filters.push(`{Company Type}="${args.companyType}"`);
  if (args.timeline) filters.push(`{Timeline}="${args.timeline}"`);
...
async function handleUpdateLead(args: any) {
  const updates: any = {};
  if (args.status) updates['Status'] = args.status;
  if (args.notes) updates['Notes'] = args.notes;
  if (args.assignedTo) updates['Assigned To'] = args.assignedTo;
...
async function handleGetAnalytics(args: any) {
  ...
  return {
    success: true,
    count: data.records.length,
    analytics: data.records.map((r: any) => ({
      id: r.id,
      ...r.fields
    }))
  };
